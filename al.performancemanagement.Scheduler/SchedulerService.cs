...
namespace al.performancemanagement.Scheduler
{
            ...
            Thread thread = new Thread(new ThreadStart( delegate
            {
                try
                {
                    while (m_isActive)
                    {
                        ...
                        {
                            Filter = f => f.Status == "Scheduled" || f.ReviewDate <= DateTime.Now.Date
                        });
                        searchEmployeeReview.Wait();

                        var employeeReviewItems = searchEmployeeReview.Result;

                        if (employeeReviewItems.Items.Count() > 0)
                        {
                            foreach(var item in employeeReviewItems.Items)
                            {
